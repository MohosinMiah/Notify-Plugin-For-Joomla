...
class plgNotiflyVirtuemartInstallerScript
{	
	...
	*/
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT extension_id from `#__extensions` WHERE `type` = 'plugin' AND `folder` = 'notifly' AND `element` = 'virtuemart'";
		$db->setQuery($sql);
		...
		if(!count($templates)){
			...
			// Insert values.
			$values = array($plugin->extension_id, $db->quote('VirtueMart PRODUCT PURCHASE'), $db->quote('virtuemart_product_purchase'), $db->quote('{{name}} from {{ country }} just purchased! **{{title_with_link}}**
