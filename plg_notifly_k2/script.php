...
class plgNotiflyK2InstallerScript
{	
	...
	*/
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT extension_id from `#__extensions` WHERE `type` = 'plugin' AND `folder` = 'notifly' AND `element` = 'k2'";
		$db->setQuery($sql);
		...
		if(!count($templates)){
			...
			// Insert values.
			$values = array($plugin->extension_id, $db->quote('NEW K2 Items Created'), $db->quote('new_k2_items'), $db->quote('{{name}} from {{ country }} just created! **{{title_with_link}}**
